...
import { createEstimateLineItemSchema, createEstimateSchema } from "@siteops/shared";
...
export function estimatesRouter(appDb: Database, env: Env): Router {
  ...
  router.get("/", async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const rows = await estimatingService.listEstimates(appDb, authUser.id, ctx, projectId);
      res.json(rows);
    } catch (err) {
      next(err);
    }
